<template>
  <div class="flex flex-col text-white md:w-1/3 mt-2">
    ...
    <div
      v-show="show"
      class="modal bg-gray-600 w-full h-full fixed top-0 right-0 left-0 z-50 flex bg-opacity-40"
    >
      <div
        class="border-neon-dark border-2 p-1 rounded-lg bg-gray-800 m-auto w-1/2 hover:border-neon-light group"
      >
        ...
        <div class="flex flex-row gap-4 font-semibold underline py-2">
          <a target="blank" :href="code">Source Code</a
          ><a target="blank" :href="link">Live Demo</a>
        </div>
        <div class="neon text-neon-light flex flex-row gap-4 text-4xl">
          <Icons
            v-for="icon in icons"
            :icon="icon"
            :key="icon.index"
